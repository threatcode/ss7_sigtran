...
typedef void sarray_t;

/* Make sure following are both of size of Word_t (as in Judy.h)
 * otherwise unexpected results may take place
 */
#define MAXLINELEN 512
typedef uint8_t [] sarray_key_t; /* String */
typedef unsigned long sarray_val_t; /* can be pointer as well (cast accordingly) */
...
typedef void (*iarray_walk_func_t)(iarray_walk_data_t *pwdata);

int iarray_walk_forward(iarray_t **iarray, iarray_walk_func_t walk_func);
int iarray_walk_backward(iarray_t **iarray, iarray_walk_func_t walk_func);
...
void iarray_walk_func_print(iarray_walk_data_t *pwdata);
