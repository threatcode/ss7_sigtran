...
#define ENC_BCD_EVEN 0x02
  int enc = strlen(gt) % 2 == 0? ENC_BCD_EVEN : ENC_BCD_ODD;
  //fprintf(stderr, "enc = %d\n", enc);

  uint8_t nextpos = 0;

  called->ai.reserved = 0x00;
  called->ai.routing_indicator = 0x00; /* route on GT (should be configurable) */
  called->ai.global_title_indicator = 0x04; /* route on GT (should be configurable) */
  called->ai.ssn_indicator = 0x01; /* route on GT (should be configurable) */
  called->ai.point_code_indicator = 0x00; /* route on GT (should be configurable) */

  called->ssn = ssn; /* HLR */
  called->gt.translation_type = 0x00;

  called->gt.encoding_scheme = enc; /* BCD */
  //called->gt.encoding_scheme = 0x02; /* BCD */
  called->gt.numbering_plan = 0x01; /* ISDN/telephony */
  called->gt.reserved = 0x00; /* ISDN/telephony */
    ...
    octet *data, uint16_t datalen)
{
  ...
  //fprintf(stderr, "*** called_gt = %s, len=%d\n", called_gt, strlen(called_gt));
  ...
  udt->variable_calling = 14+1; /* currently hard coded, later on will be configurable */
  //udt->variable_data = 25+1; /* currently hard coded, later on will be configurable */
  udt->variable_data = 25+2; /* currently hard coded, later on will be configurable */
...
sccp_data_udt_t *sccp_octet2udt(octet *buf, uint16_t buflen, uint32_t *nextpos)
{
  ...
  */
  ...
  memcpy(udt->called.gt.digits, buf+i, udt->called_len-5);
  i += udt->called_len-5;
  ...
  */
  ...
  memcpy(udt->calling.gt.digits, buf+i, udt->calling_len-5);
  i += udt->calling_len-5;
