...
AnyTimeInterrogationRes_t *ati_decode(const void *buf, size_t len)
{
  asn_dec_rval_t rval;
  AnyTimeInterrogationRes_t *pdu = NULL; /* Note this 0! (Forgetting to properly initialize the pointing to a destination structure is a major source of support requests) */
...
#else
    /* Free partially decoded data */
    CRITICAL("TCAP Decode failed, Freeing partially parased data structure\n");
    tcap_free(pdu);
    pdu = NULL;
#endif
  }
...
void ati_free(AnyTimeInterrogationRes_t *pdu)
{
  if (pdu)
    asn_DEF_AnyTimeInterrogationRes.free_struct(&asn_DEF_AnyTimeInterrogationRes, pdu, 0);
}
...
void ati_print(AnyTimeInterrogationRes_t *pdu)
{
  if (pdu)
    xer_fprint(stdout, &asn_DEF_AnyTimeInterrogationRes, pdu);
}
