...
/* LBS QUERY (ATI) */
/* flow always 'begin' */
/* if msg==__CGI__ then issue ATI Query*/
/* notify_only ignored for ATI Query (LBS) */
int tcap_msg_begin_mt(sigtran_t *st, tcap_session_t *tcsess, const char *msg, const char *msisdn, uint8_t notify_only)
{
  ...
  if (strcmp(msg, "__CGI__") == 0) is_ati = 1; /* Issue ATI Query */

  tcm = MYCALLOC(1, sizeof(*tcm));

  if (is_ati) tcsess->invokeid = 0;
  else tcsess->invokeid = -128;
  if (msisdn) {
  ...
  if (is_ati) {
    ext = ati_dialogue_request_build(); /* destref = msisdn, origref = hlr_gt */
  } else {
  ...
  if (is_ati) {
    comp = ati_session_comp_build(tcsess, st->ugw_gt, msisdn, flow, notify_only);
  } else {
    comp = ussd_session_comp_build(tcsess, msg, msisdn, flow, notify_only);
  }
  ASN_SEQUENCE_ADD(&cp->list, comp);
  if (is_ati) {
    cp->list.free = ati_session_comp_free;
  } else {
  ...
  const char *sccp_called_party_number = st->hlr_gt;
  if (st->sccp_gt_use_msisdn == 1) {
    sccp_called_party_number = msisdn;
  }
  sccp_data_udt_t *udt = sccp_build_udt(st->hlr_ssn, sccp_called_party_number,
...
size_t sigtran_tcap_stat_dump(sigtran_t *st, char *buf, size_t buflen)
{
  ...
  uint16_t i = 0;
  uint32_t ndialogs = 0;
  uint32_t max_dialogs = nmaps * 65536;
  char started[32];
  char last_recv[32];
  ...
  ctime_r(&st->started, started);
  if (st->last_recv > 0) ctime_r(&st->last_recv, last_recv);
  else strcpy(last_recv, "N/A");

  memset(buf, 0, buflen);
  return snprintf(buf, buflen-1,
      "Gateway started: %sLast request received: %s"
      "%u/%u sessions being handled in %u MAP License\n",
      started, last_recv,
      ndialogs, max_dialogs, nmaps);
}
...
//  mystrncpy(buf, buflen, err_resp, &nr);
  ...
  if (strncmp(request_path, "/stat", 5) == 0) { /* show statistics */
    nr = sigtran_tcap_stat_dump(st, buf, buflen);
    return nr;
  } else if (strncmp(request_path, "/shutdown", 9) == 0) { /* shutdown (restart by script) Gateway */
    st->system_shutdown = 1;
    nr = snprintf(buf, buflen-1, "Gateway Shutdown request [%s] received. Will be executed in one minute!\n", request_path);
    return nr;
  }

  DTRACE("Request=%s\n", request_path);

  if ((strncmp(request_path, "/sigtran", 8) != 0) || (extract_tokens(query_string, msisdn, sizeof(msisdn), text, sizeof(text), &notify_only) < 0)) {
    mystrncpy(buf, buflen, err_resp, &nr);
  } else {
