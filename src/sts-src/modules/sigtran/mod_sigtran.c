...
void sigtran_state_ssnm(sigtran_t *s)
{
  ...
  switch (s->sm->head.mtype) {
	...
	if (t && t->tag == 0x0012) { /* Affected Point Code */
	  ...
	  pc = ntohl(pc);
	  DTRACE("Mask: 0x%x (%u)\n", mode, mode);
	  if (s->sm->head.mtype == M3UA_MSG_TYPE_SSNM_DAVA) {
	    TTRACE("[SSNM: DAVA] Point Code: 0x%x (%u)\n", pc, pc);
	  } else {
	    TTRACE("[SSNM: DUNA] Point Code: 0x%x (%u)\n", pc, pc);
	  }
	}
	l = l->next;
      }
      break;
    default:
      TTRACE("[SSNM] mclass=0x%x (%u), mtype=0x%x (%u)\n",
	  s->sm->head.mclass, s->sm->head.mclass,
	  s->sm->head.mtype, s->sm->head.mtype);
      break;
  }
...
void sigtran_state_aspsm(sigtran_t *s)
{
  ...
  switch (s->sm->head.mtype) {
	...
	if (t && t->tag == 0x0009) { /* BEAT */
	  ...
	  TTRACE("[ASPSM: BEAT_ACK] m3ua_hb = 0x%x [%d]\n", s->m3ua_hb, s->m3ua_hb);
	...
	l = l->next;
      ...
      break;
    default:
      TTRACE("[ASPSM] mclass=0x%x (%u), mtype=0x%x (%u)\n",
	  s->sm->head.mclass, s->sm->head.mclass,
	  s->sm->head.mtype, s->sm->head.mtype);
      break;
  }
...
void sigtran_state_asptm(sigtran_t *s)
{
  ...
  switch (s->sm->head.mtype) {
    default:
      TTRACE("[ASPTM] mclass=0x%x (%u), mtype=0x%x (%u)\n",
	  s->sm->head.mclass, s->sm->head.mclass,
	  s->sm->head.mtype, s->sm->head.mtype);
      break;
  }
...
void sigtran_state_machine(sigtran_t *s)
{
  ...
  while (1) {
    ...
    if (bufpos <= 0) break; // no more to process
	...
	exit(EXIT_FAILURE);
      ...
      //sleep(1); // just to check, in case we fall into infinite loop
      continue;
...
static void *sigtran_m3ua_hb_process(void *s, void *sys)
{
  ...
  if (st->system_shutdown) {
    CRITICAL("Gateway Shutdown Requested, exiting...\n");
    close(st->fd);
    exit(EXIT_SUCCESS);
  }
...
#define SIGTRAN_M3UA_TIMER_MAX		3
  ...
  /* run your timer jobs here */
  if (!st->aspup_sent) {
    sigtran_send_ASPUP(st);
  }
...
int module_start(system_t *sys)
{
  ...
  uint8_t sccp_gt_use_msisdn = 1;/* USE MSISDN AS SCCP GT CALLED PARTY (INSTEAD OF HLR GT) */
...
#define CFGFILE "sigtran_test.cfg"
  ...
  while (fgets(cbuf, sizeof(cbuf)-1, cfgfp)) {
	...
	*cptr = '\0';
    ...
    if (strcmp(ckey, "msc_ip") == 0) {
      ...
      strcpy(appurl, cptr);
    } else if (strcmp(ckey, "sccp_gt_use_msisdn") == 0) {
      sccp_gt_use_msisdn = (uint8_t) atoi(cptr);
    }
  ...
  if (s) {
    ...
    st->ugw_ssn = ugw_ssn;
    st->hlr_ssn = hlr_ssn;
    st->sccp_gt_use_msisdn = sccp_gt_use_msisdn;
