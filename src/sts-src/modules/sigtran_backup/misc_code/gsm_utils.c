...
#if 0
	if (nchars && (user_data[nchars-1] >> 1) == '\r' && (user_data[nchars-1] & 0x01) == '\0' && user_data[nchars-1] & 0xe0) { /* <CR> found padded by 0 */
	  fprintf(stderr, "Last septet is <CR>, removing it.\n");
		text[--nchars] = '\0';
	}
#endif
	/* FIXME: HLR in Teletalk is not really adding additional <CR>, so we shouldn't cut it */
	/* remove last <CR>, if it fits up to the end of last octet */
	if (nchars && (user_data[gsm_get_octet_len(length) - 1] >> 1) == '\r') {
	  /* need to double check for <CR> */
	  fprintf(stderr, "Last septet is <CR>, removing it.\n");
		text[--nchars] = '\0';
	}
