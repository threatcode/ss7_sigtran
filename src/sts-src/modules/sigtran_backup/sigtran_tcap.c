...
int sigtran_tcap_comp_parse(sigtran_tcap_info_t *sinfo)
{
  ...
  comp = sinfo->comp;
  if (!comp) goto end; /* 'end'ed (system failure)  sessions will match the above condition, so won't reach here */
  ...
  if (opCode && (opCode->present == OPERATION_PR_localValue)) {
    ...
    if (tcsess->flow == TCMessage_PR_begin) {
      tcsess->end_opcode = tcsess->opcode;
      tcsess->end_invokeid = tcsess->invokeid;
    }
    /* parse parameter */
    sinfo->param = parameter;
    if (parameter) ret = sigtran_tcap_parameter_parse(sinfo);
  } else {
