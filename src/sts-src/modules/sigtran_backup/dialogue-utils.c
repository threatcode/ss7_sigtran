...
ExternalPDU_t *camel_ati_dialogue_request_build(void)
{
  FTRACE();
...
#define MAX_DIRECT_REFERENCE_LEN	16
  ...
  if (er.encoded > 0) {
    ...
    DTRACE("Failed to encode map dialogue pdu Data\n");
    goto err;
  }
  ...
  if (er.encoded > 0) {
    ...
    DTRACE("Failed to encode object identifier Data\n");
    goto err;
  }
...
//  camel_ati_dialogue_request_free(ext);
//  ext = NULL;
  /*
